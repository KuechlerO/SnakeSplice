...
DEXSeqDataSetFromFeatureCounts <- function (countfile, sampleData,
...

  {
        ...
        # %>% is a chain-operator given by the magrittr-package
        # arrange: Sorting
        # V1, ... are default names assigned by R to columns
        # Then select only V1?
        ...
        if (!is.null(flattenedfile)) {
                ...
                aggregates$gene_id <- substr(aggregates$gene_id,1,max(nchar(id)))
